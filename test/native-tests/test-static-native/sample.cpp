...
#define __int8      char 
#define TESTSTRING char
#define _cdecl 
#define _u(s) u8##s
template<typename T, size_t l>
constexpr size_t _countof(T (&array)[l]) { return l; }
...
//TODO: these are just existing to get the test passing. 
//a better approach to strings has to be found and wchar isn't portable
//as while these are encoded as utf8, it will fall a few tests if we try and load something other than ascii
JsErrorCode JsRunScript(const char * str, JsSourceContext sourceContext, const char * sourceUrl, JsValueRef* result) {
    JsValueRef sourceLocation;
    JsValueRef scriptSource;
    JsCreateString(sourceUrl, strlen(sourceUrl), &sourceLocation);
...
JsErrorCode JsParseScript(const char * str, JsSourceContext sourceContext, const char * sourceUrl, JsValueRef* result) {
    JsValueRef sourceLocation;
    JsValueRef scriptSource;
    JsCreateString(sourceUrl, strlen(sourceUrl), &sourceLocation);
    std::string script(str);
    JsCreateExternalArrayBuffer((void *)script.c_str(), script.length(),  nullptr, nullptr, &scriptSource);
    return JsParse(scriptSource, sourceContext, sourceLocation, JsParseScriptAttributeNone, result);
}
/* see the byte code tests for why this is removed
JsErrorCode JsSerializeScript(const char *script, BYTE *buffer, unsigned int *bufferSize) 
{
    JsValueRef scriptSource;
    std::string s(script);
    JsCreateExternalArrayBuffer((void *)s.c_str(), s.length(),  nullptr, nullptr, &scriptSource);
    return JsSerialize(scriptSource, buffer, JsParseScriptAttributeNone);
}
*/
...
JsErrorCode JsGetPropertyNameFromId(JsPropertyIdRef propertyId, std::string& result) {
   return JsCopyTo(propertyId, result, JsCopyPropertyId);
}

JsErrorCode JsStringToString(JsValueRef propertyId, std::string& result) {
    return JsCopyTo(propertyId, result, JsCopyString);
}
...
namespace JsRTApiTest
{
    ...
    //TODO JW: Find some way to be able to toggle this behaviour so we can test both sets of api on windows
    JsErrorCode CreatePropertyID(std::string prop, JsPropertyIdRef *propertyId)
    {
        return JsCreatePropertyId(prop.c_str(), prop.length(), propertyId);
    }
    ...
    void ReferenceCountingTest(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
    {
        ...
        REQUIRE(CreatePropertyID("foo", &foo) == JsNoError);
    ...
    void ObjectsAndPropertiesTest1(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
    {
        ...
        REQUIRE(JsCreateString(v1.c_str(), v1.length(), &value1) == JsNoError);

        JsValueRef value2 = JS_INVALID_REFERENCE;
        REQUIRE(JsCreateString(v1.c_str(), v1.length(), &value2) == JsNoError);
    ...
    void ObjectsAndPropertiesTest2(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
    {
        ...
        std::string script;
        REQUIRE(FileLoadHelpers::LoadScriptFromFile("../Scripts/ObjectsAndProperties2.js", script) == LoadScriptResult::S_OK);
        REQUIRE(script.length() > 0);

        REQUIRE(JsParseScript(script.c_str(), JS_SOURCE_CONTEXT_NONE, _u(""), &function) == JsNoError);
        ...
        std::string str;
        REQUIRE(JsStringToString(asString, str) == JsNoError);
        REQUIRE(str == _u("white"));
    }
    ...
    void ArrayAndItemTest(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
    {
        ...
        JsValueRef value1 = JS_INVALID_REFERENCE;
        REQUIRE(JsCreateStringFromString(_u("value1"), &value1) == JsNoError);
    ...
    void EqualsTest(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
    {
        ...
        std::string v1("1");
        JsValueRef value1 = JS_INVALID_REFERENCE;
        REQUIRE(JsCreateStringFromString(v1, &stringa) == JsNoError);

        JsValueRef stringb = JS_INVALID_REFERENCE;
        REQUIRE(JsCreateStringFromString(v1, &stringb) == JsNoError);
    ...
    void LanguageTypeConversionTest(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
    {
        ...
        std::string pi("3.141592");
        REQUIRE(JsCreateStringFromString(pi, &asString) == JsNoError);
        ...
        REQUIRE(JsCreateStringFromString(trueStr, &asString) == JsNoError);
    ...
    JsValueRef CALLBACK ExternalFunctionPreScriptAbortionCallback(JsValueRef /* function */, bool /* isConstructCall */, JsValueRef * args /* args */, USHORT /* cargs */, void * /* callbackState */)
    {
        ...
        REQUIRE(JsStringToString(args[0], scriptText) == JsNoError);
        REQUIRE(JsRunScript(scriptText.c_str(), JS_SOURCE_CONTEXT_NONE, _u(""), &result) == JsErrorScriptTerminated);
        return nullptr;
    }
    ...
    JsValueRef CALLBACK ExternalFunctionPostScriptAbortionCallback(JsValueRef /* function */, bool /* isConstructCall */, JsValueRef * args /* args */, USHORT /* cargs */, void * /* callbackState */)
    {
        ...
        REQUIRE(JsStringToString(args[0], scriptText) == JsNoError);
        REQUIRE(JsRunScript(scriptText.c_str(), JS_SOURCE_CONTEXT_NONE, _u(""), &result) == JsErrorInDisabledState);
        return nullptr;
    }
    ...
    void ErrorHandlingTest(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
    {
        // Run a script to setup some globals
        std::string script;
        REQUIRE(FileLoadHelpers::LoadScriptFromFile("../Scripts/ErrorHandling.js", script) == LoadScriptResult::S_OK);
        REQUIRE(script.length() > 0);

        REQUIRE(JsRunScript(script.c_str(), JS_SOURCE_CONTEXT_NONE, _u(""), nullptr) == JsNoError);
    ...
    void ScriptCompileErrorTest(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
    {
        ...
        REQUIRE(JsStringToString(value, str) == JsNoError);
        CHECK(str == "Syntax error");
        ...
        REQUIRE(JsGetProperty(error, property, &value) == JsNoError);
        REQUIRE(JsStringToString(value, str) == JsNoError);
        CHECK(str == _u("The use of a keyword for an identifier is invalid"));
    ...
    {
        std::string script(_u("x = { a: \"abc\", b: \"def\", c: \"ghi\" }; x"));
        JsValueRef result = JS_INVALID_REFERENCE;
        JsValueRef propertyNames = JS_INVALID_REFERENCE;

        REQUIRE(JsRunScript(script.c_str(), JS_SOURCE_CONTEXT_NONE, _u(""), &result) == JsNoError);
        ...
        for (int index = 0; index < 3; index++)
        {
            ...
            REQUIRE(JsStringToString(nameValue, name) == JsNoError);

            CHECK(name.length() == 1); //todo this won't work for any utf-8 chars outside of ascii range.
    ...
    void SymbolTests(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
    {
        ...
        REQUIRE(JsCreateStringFromString(_u("abc"), &string1) == JsNoError);
...
/* TODO: jared there is something strange about the seralizedocs and what is exposed in the header look into */

//     void ByteCodeTest(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
//     {
//         std::string script(_u("function test() { return true; }; test();"));
//         JsValueRef result = JS_INVALID_REFERENCE;
//         JsValueType type;
//         bool boolValue;
...
//         REQUIRE(JsSerializeScript(script.c_str(), compiledScript, &scriptSize) == JsNoError);
//         compiledScript = new BYTE[scriptSize];
//         unsigned int newScriptSize = scriptSize;
//         REQUIRE(JsSerializeScript(script.c_str(), compiledScript, &newScriptSize) == JsNoError);
//         CHECK(newScriptSize == scriptSize);
//         REQUIRE(JsRunSerializedScript(script.c_str(), compiledScript, JS_SOURCE_CONTEXT_NONE, _u(""), &result) == JsNoError);
//         REQUIRE(JsGetValueType(result, &type) == JsNoError);
...
//     }
 ...
 void ContextCleanupTest(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
    ...
    void SetPrototypeTest(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
    {
        ...
        std::string obj1a(_u("obj1.a"));
        REQUIRE(JsCreateStringFromString(obj1a, &obj1_a_value) == JsNoError);
        ...
        std::string obj1b(_u("obj1.b"));
        REQUIRE(JsCreateStringFromString(obj1b, &obj1_b_value) == JsNoError);
        ...
        std::string obj2x(_u("obj2.x"));
        REQUIRE(JsCreateStringFromString(obj2x, &obj2_x_value) == JsNoError);
        ...
        std::string obj1y(_u("obj1.y"));
        REQUIRE(JsCreateStringFromString(obj1y, &obj2_y_value) == JsNoError);
        ...
        std::string obj1z(_u("obj1.y"));
        REQUIRE(JsCreateStringFromString(obj1z, &obj2_z_value) == JsNoError);
    ...
    void CreatePromiseTest(JsRuntimeAttributes attributes, JsRuntimeHandle runtime)
    {
        JsValueRef result = JS_INVALID_REFERENCE;
        JsValueType valueType;
        JsValueRef task = JS_INVALID_REFERENCE;
    ...
    TEST_CASE("ApiTest_CreatePromiseTest", "[ApiTest]")
    {
        JsRTApiTest::RunWithAttributes(JsRTApiTest::CreatePromiseTest);
    }
    ...
    /* TODO: jared this has some windows only events that depends on getting pal.h into this code base
    ...
    */
//TODO: jared the docs here don't actually match what is being called. I am guessing this is something to do with windows.
//JsSetModuleHostInfo
    ...
    //     JsErrorCode errorCode = JsStringToString(specifier, specifierStr);
    //     REQUIRE(errorCode == JsNoError);
    //     REQUIRE(specifierStr == _u("foo.js"));
    ...
    //     REQUIRE(JsCreateStringFromString(_u(""), &specifier) == JsNoError);
